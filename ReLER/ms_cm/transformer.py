...
class CrossEncoder(nn.Module):
    ...
    def forward(
        ...
        for layer in self.layers:
            vid_embedding, txt_embedding, co_attention_probs = layer(
                vid_embedding,
                vid_attention_mask,
                txt_embedding,
                txt_attention_mask,
            ...
            for coa_prob in co_attention_probs:
                vid_embedding = vid_embedding + (coa_prob * 0.0).sum()
...
class Cross_self_attention(nn.Module):
    ...
    def forward(
        self, src_vid, mask_vid, src_txt, mask_txt, vid_pos_embed, txt_pos_embed
    ):
        if self.cross_first:
            return self.forward_cross_first(
                src_vid, mask_vid, src_txt, mask_txt, vid_pos_embed, txt_pos_embed
            )
        else:
            return self.forward_self_first(
                src_vid, mask_vid, src_txt, mask_txt, vid_pos_embed, txt_pos_embed
            )

    def forward_cross_first(
        self, src_vid, mask_vid, src_txt, mask_txt, vid_pos_embed, txt_pos_embed
    ):
        """
        Args:
        ...
        cross_vid_output, cross_txt_output = self.cross_encoder(
            vid_embedding,
            extended_mask_vid,
            txt_embedding,
            extended_mask_txt,
        ...
        transformer_encoder_src = torch.cat(
            [cross_vid_output, cross_txt_output], dim=1
        )  # (bsz, L_vid+L_txt, d)

        transformer_encoder_src = transformer_encoder_src.permute(
            1, 0, 2
        )  # (L, batch_size, d)
        transformer_encoder_pos = transformer_encoder_pos.permute(
            1, 0, 2
        )  # (L, batch_size, d)

        transformer_encoder_output = self.encoder(
            transformer_encoder_src,
            src_key_padding_mask=transformer_encoder_mask,
            pos=transformer_encoder_pos,
        )

        transformer_encoder_output = transformer_encoder_output.transpose(
            0, 1
        )  # (batch_size, L, d)\

        txt_output = transformer_encoder_output[
            :, src_vid.shape[1] :
        ]  # (bsz, L_txt, d)
        vid_output = transformer_encoder_output[
            :, : src_vid.shape[1]
        ]  # (bsz, L_vid, d)
        return vid_output, txt_output

    def forward_self_first(
        self, src_vid, mask_vid, src_txt, mask_txt, vid_pos_embed, txt_pos_embed
    ):
        """
        Args:
        ...
        transformer_encoder_output_txt = transformer_encoder_output[
            :, src_vid.shape[1] :
        ]  # (bsz, L_txt, d)
        transformer_encoder_output_vid = transformer_encoder_output[
            :, : src_vid.shape[1]
        ]  # (bsz, L_vid, d)
        ...
        if not self.cross_text_use_ori:
            cross_vid_output, cross_txt_output = self.cross_encoder(
                transformer_encoder_output_vid,
                extended_mask_vid,
                transformer_encoder_output_txt,
                extended_mask_txt,
                co_attention_mask,
                use_co_attention_mask,
            )  # (L, batch_size, d)
...
class Onlyvtrans_encoder(nn.Module):
    def __init__(
        ...
        super().__init__()
        onlyv_encoder_layer = TransformerEncoderLayer(
            d_model, nhead, dim_feedforward, dropout, activation, normalize_before
        )
        encoder_norm = nn.LayerNorm(d_model) if normalize_before else None
        self.onlyv_encoder = TransformerEncoder(
            onlyv_encoder_layer, num_onlyv_layers, encoder_norm
        )
    ...
    def forward(self, src_vid, mask_vid, vid_pos_embed):
        ...
        only_vid_embedding = self.onlyv_encoder(
            only_vid_embedding,
            src_key_padding_mask=onlyv_encoder_mask,
            pos=onlyv_encoder_pos,
        )
...
class Cross_self_attention_fixcross_addonlyvtrans(nn.Module):
    ...
    def forward_cross_first(
        ...
        if self.num_onlyv_layers != 0:
            ...
            src_vid = self.encoder(
                src_vid, src_key_padding_mask=onlyv_encoder_mask, pos=onlyv_encoder_pos
            )
