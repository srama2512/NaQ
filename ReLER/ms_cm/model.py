...
from ms_cm.transformer import (
    build_transformer,
    build_transformer_encoder,
    build_cross_self_encoder_fixcross,
)
...
def pad_text_tenor_list(
    sequences, dtype=torch.long, device=torch.device("cuda"), fixed_length=None
):
...
class MS_CM(nn.Module):
    ...
    def __init__(
        ...
        vid_dim,
        input_dropout,
        conditioned_span_pred_dict,
        video_frame_contrastive_loss=False,
        contrastive_hdim=64,
        max_v_l=75,
        use_txt_pos=False,
        n_input_proj=2,
        ...
        if video_frame_contrastive_loss:
            ...
            self.contrastive_align_projection_txt = nn.Linear(
                hidden_dim, contrastive_hdim
            )
            self.contrastive_align_projection_vid = nn.Linear(
                hidden_dim, contrastive_hdim
            )
    ...
    def encode_text(self, src_raw_txt, device):
        # Reference: https://github.com/openai/CLIP/blob/d50d76daa670286dd6cacf3bcd80b5e4823fc8e1/clip/model.py#L343-L356
        model = self.clip_encoder[0]
        with torch.no_grad():
            text = clip.tokenize(src_raw_txt).to(device)  # [batch_size, n_ctx]
            ...
            x = model.ln_final(x).type(
                model.dtype
            )  # [batch_size, n_ctx, transformer.width]

            # Get sentence features
            x = x @ model.text_projection  # [batch_size, n_ctx, dim]

            max_idx = text.argmax(dim=1).max().item()
            src_txt = x[:, :max_idx].float()  # [batch_size, max_idx, dim]
            src_txt_mask = torch.zeros(text.shape[0], max_idx).to(device)
            for i in range(text.shape[0]):
                max_idx_i = text[i].argmax().item()
                src_txt_mask[i, :max_idx_i] = 1

        return src_txt, src_txt_mask

    def forward(self, src_raw_txt, src_vid_total, src_vid_mask_total, scale_mask):
        """
        - src_raw_txt: a list of text strings
        ...
        """
        device = src_vid_total[0].device
        if self.clip_encoder is None:
            clip_encoder, _ = clip.load("ViT-B/16", device=device)
            self.clip_encoder = [clip_encoder]
            self.clip_encoder[0].eval()
        src_txt, src_txt_mask = self.encode_text(src_raw_txt, device)
        ...
        for src_vid, src_vid_mask in zip(src_vid_total, src_vid_mask_total):
            ...
            vid_mem_split, txt_mem = self.cross_self_encoder(
                src_vid, src_vid_mask, src_txt, src_txt_mask, pos_vid, pos_txt
            )
            vid_mem_split = vid_mem_split + (txt_mem * 0.0).sum()
        ...
        for vid_mem_split_tmp, NPM_split_score_tmp in zip(
            vid_mem_total_list, NPM_split_score_list
        ):
            vid_mem_withNPM_split = vid_mem_split_tmp * NPM_split_score_tmp.unsqueeze(
                -1
            )
        ...
        out["saliency_scores"] = self.saliency_proj(vid_mem_withNPM_total).squeeze(
            -1
        )  # (bsz, L_vid)

        if self.video_frame_contrastive_loss:
            proj_txt_mem = F.normalize(
                self.contrastive_align_projection_txt(src_txt), p=2, dim=-1
            )
            proj_vid_mem = F.normalize(
                self.contrastive_align_projection_vid(vid_mem_withNPM_total),
                p=2,
                dim=-1,
            )
            out.update(dict(proj_txt_mem=proj_txt_mem, proj_vid_mem=proj_vid_mem))
        ...
        start_logits, end_logits = self.predictor(
            highlight_feature, mask=src_vid_mask_final
        )
...
class SetCriterion(nn.Module):
    ...
    def __init__(self, weight_dict, losses, temperature, max_v_l, saliency_margin=1):
    ...
    def loss_start_end(self, outputs, targets, log=True):
        start_loss = nn.CrossEntropyLoss(reduction="mean")(
            outputs["start_logits"], targets["start_label"]
        )
        end_loss = nn.CrossEntropyLoss(reduction="mean")(
            outputs["end_logits"], targets["end_label"]
        )
    ...
    def loss_video_frame_contrastive(self, outputs, targets, log=True):
        ...
        logits = torch.einsum(
            "bmd,bnd->bmn", normalized_vid_embed, normalized_text_embed
        )  # (bsz, #queries, #tokens)

        logits = logits * txt_mask.unsqueeze(1)
        logits = (
            logits.sum(2) / self.temperature / txt_mask.unsqueeze(1).sum(2)
        )  # (bsz, #queries)
        positive_map = torch.zeros_like(logits, dtype=torch.bool)

        for batch_idx, (start_label_tmp, end_label_tmp) in enumerate(
            zip(start_label, end_label)
        ):
    ...
    def forward(self, outputs, targets):
        ...
        if "aux_outputs" in outputs:
            ...
            for i, aux_outputs in enumerate(outputs["aux_outputs"]):
                ...
                for loss in self.losses:
                    if (
                        "saliency" == loss or "highlight" == loss
...
def build_model(args):
    ...
    if args.video_frame_contrastive_loss:
        ...
        weight_dict[
            "loss_video_frame_contrastive"
        ] = args.video_frame_contrastive_loss_coef
