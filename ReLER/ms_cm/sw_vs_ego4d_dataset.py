...
class Ego4d_dataset(Dataset):
    ...
    def __init__(
        ...
        if self.mode == "eval" or self.mode == "test":
            ...
            assert nar_rand_exp_factor == -1.0
            assert nar_rand_translate == False
    ...
    def __getitem__(self, index):
        ...
        model_inputs = dict()
        model_inputs["query_text"] = record["query"]
        ...
        if self.use_video:
            (
                ...
                s_ind,
                e_ind,
            ) = self._get_video_feat_by_vid(
                record, record_random
            )  # (Lv, Dv)
        ...
        s_ind, e_ind = self.expand_narration_window(
            self.record_query, s_ind, e_ind, record["v_len"]
        )
    ...
    def _get_video_feat_by_vid(self, record, record_random):
        ...
        if self.use_sw and self.mode == "train":
            cur_sw_len_ratio = random.uniform(
                self.sw_len_ratio_start, self.sw_len_ratio_end
            )
        ...
        for idx, _feat_dir in enumerate(self.v_feat_dirs):
            ...
            if self.use_sw and self.mode == "train":
                if idx == 0:
                    ...
                    s_ind, e_ind = math.floor(
                        min(gt_s_time / record["duration"], 1.0) * (total_feat_len - 1)
                    ), math.ceil(
                        min(gt_e_time / record["duration"], 1.0) * (total_feat_len - 1)
                    )
                    ...
                    if total_feat_len_mulanchor <= ori_clip_len:
                        if ori_clip_len == total_feat_len:
                            ...
                            total_feat_len_mulanchor = random.randrange(
                                ori_clip_len, total_feat_len
                            )
                    rand_start_choice = max(0, e_ind - total_feat_len_mulanchor)
                    rand_end_choice = min(
                        s_ind, total_feat_len - total_feat_len_mulanchor
                    )

                    rand_s_idx = (
                        random.randrange(rand_start_choice, rand_end_choice)
                        if total_feat_len > total_feat_len_mulanchor
                        and rand_start_choice < rand_end_choice
                        else rand_end_choice
                    )
                    rand_e_idx = min(
                        rand_s_idx + total_feat_len_mulanchor, total_feat_len
                    )
        ...
        # Adjust for slightly different feature lengths
        max_feat_len = max([rv.shape[0] for rv in v_feat_list])
        for vi, vf in enumerate(v_feat_list):
            if len(vf) < max_feat_len:
                vf = np.pad(vf, ((0, max_feat_len - len(vf)), (0, 0)), "constant")
                v_feat_list[vi] = vf
        ...
        if cur_vs:
            ...
            # Adjust for slightly different feature lengths
            max_feat_len = max([rv.shape[0] for rv in random_v_feat_list])
            for vi, vf in enumerate(random_v_feat_list):
                if len(vf) < max_feat_len:
                    vf = np.pad(vf, ((0, max_feat_len - len(vf)), (0, 0)), "constant")
                    random_v_feat_list[vi] = vf
        ...
        v_feat = visual_feature_sampling(v_feat, max_num_clips=self.max_v_l)
        vlen = v_feat.shape[0]
        if self.use_sw and self.mode == "train":
            s_ind, e_ind = math.floor(
                min(gt_s_time / (pre_v_feat_len * time_unit), 1.0) * (vlen - 1)
            ), math.ceil(
                min(gt_e_time / (pre_v_feat_len * time_unit), 1.0) * (vlen - 1)
            )
...
def Ego4d_collate(batch):
    ...
    for idx in range(batch_size):
        st, et = batch_s_ind[idx], batch_e_ind[idx]
        for i in range(cur_scale):
            if (st >= start_point_list[i] and st < end_point_list[i]) or (
                et >= start_point_list[i] and et < end_point_list[i]
            ):
                NPM_labels[idx][i] = 1
...
def prepare_batch_inputs(batched_model_inputs, device, non_blocking=False, cur_scale=1):
    ...
    src_vid_split = torch.split(
        batched_model_inputs["video_feat"][0].to(device, non_blocking=non_blocking),
        split_len,
        dim=1,
    )
    src_vid_mask_split = torch.split(
        batched_model_inputs["video_feat"][1].to(device, non_blocking=non_blocking),
        split_len,
        dim=1,
    )
    model_inputs = dict(
        src_raw_txt=batched_model_inputs["query_text"],
        src_vid_total=src_vid_split,
        src_vid_mask_total=src_vid_mask_split,
        scale_mask=batched_model_inputs["video_feat"][2].to(
    ...
    targets["highlight_label"] = batched_model_inputs["highlight_label"].to(
        device, non_blocking=non_blocking
    )
    targets["NPM_labels"] = batched_model_inputs["NPM_labels"].to(
        device, non_blocking=non_blocking
    )
    targets["start_label"] = batched_model_inputs["start_label"].to(
        device, non_blocking=non_blocking
    )
    targets["end_label"] = batched_model_inputs["end_label"].to(
        device, non_blocking=non_blocking
    )
