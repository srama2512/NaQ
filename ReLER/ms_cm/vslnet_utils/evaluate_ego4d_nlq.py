...
def display_results(results, mIoU, thresholds, topK, title=None):
    ...
    display_data[0].insert(0, f"meanR1")
    results *= 100
    mIoU *= 100
    display_data.append(
        [
    ...
    display_data[1].insert(0, f"{(results[0][0] + results[1][0])/2:.02f}")
    table = terminaltables.AsciiTable(display_data, title)
    for ii in range(len(thresholds) * len(topK) + 1):
        table.justify_columns[ii] = "center"
    return table.table


def display_results_addmeanr1(results, mIoU, thresholds, topK, title=None):
    display_data = [
        [f"Rank@{ii}\nmIoU@{jj}" for ii in topK for jj in thresholds] + ["mIoU"]
    ]
...
def get_results_dict(results, mIoU, thresholds, topK, scale=True):
    if scale:
