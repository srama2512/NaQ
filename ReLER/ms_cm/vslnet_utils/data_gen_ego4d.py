...
class EpisodicNLQProcessor:
    def __init__(self, thres_in_train=-1):
        super(EpisodicNLQProcessor, self).__init__()
        self.idx_counter = 0
        self.thres_in_train = thres_in_train
    ...
    def process_data_tan(self, data, scope):
        ...
        for vid, data_item in tqdm(
            ...
            for timestamp, exact_time, sentence, ann_uid, query_idx in zipper:
                ...
                if scope == "train":
                    if end_time - start_time <= self.thres_in_train:
                        continue
                record = {
...
def dataset_gen_bert(data, vfeat_lens, max_pos_len, scope, num_workers=1):
    # Worker method for multiprocessing.
    def worker(worker_data, vfeat_lens, max_pos_len, scope, worker_id, output_q):
        worker_dataset = list()
        description = f"process {scope} data [{worker_id}]"
        # Cache candidates
        max_num_units = max([v for k, v in vfeat_lens.items()])
        s_units = np.arange(0, max_num_units).astype(np.float32)
        e_units = np.arange(1, max_num_units + 1).astype(np.float32)
        candidate_units = np.stack(
            [
                np.repeat(s_units[:, None], repeats=max_num_units, axis=1),
                np.repeat(e_units[None, :], repeats=max_num_units, axis=0),
            ],
            axis=2,
        )
        for record in tqdm(worker_data, total=len(worker_data), desc=description):
            ...
            s_ind, e_ind, _ = time_to_index(
                ...
                vfeat_lens[vid],
                record["duration"],
                cache_candidate_units=candidate_units,
            )
            result = {
                "sample_id": record["sample_id"],
                "vid": record["vid"],
                ...
                "e_ind": int(e_ind),
                "v_len": vfeat_lens[vid],
                "exact_s_time": record["exact_s_time"],
                "exact_e_time": record["exact_e_time"],
                "annotation_uid": record["annotation_uid"],
                "query_idx": record["query_idx"],
            }
            worker_dataset.append(result)
    ...
    for worker_id in range(num_workers):
        allotment = data[worker_id * chunk_size : (worker_id + 1) * chunk_size]
        inputs = (allotment, vfeat_lens, max_pos_len, scope)
        inputs += (worker_id, output_q)
        process = multiprocessing.Process(target=worker, args=inputs)
        jobs.append(process)
        process.start()
...
def gen_or_load_dataset(configs):
    ...
    data_dir = os.path.join(configs.vslnet_datapath, "dataset", configs.task)
    feature_dir = os.path.join(configs.vslnet_datapath, "features", configs.fv)
    ...
    # load data
    processor = EpisodicNLQProcessor(configs.thres_in_train)
