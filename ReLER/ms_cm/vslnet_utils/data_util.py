...
def load_video_features(root, max_position_length, use_normforvideofeature):
    video_features = dict()
    extension = "*.pt"
    ...
    for filename in tqdm(filenames, total=len(filenames), desc="load video features"):
        ...
        if use_normforvideofeature:
            # print('use_normforvideofeature')
            feature = l2_normalize_np_array(feature)
...
def load_video_features_concatCLIPfeature(
    root, max_position_length, CLIPfeature_save_path
):
    video_features = dict()
    extension = "*.pt"
    count = 0
    ...
    for filename in tqdm(filenames, total=len(filenames), desc="load video features"):
        ...
        try:
            CLIP_feature = torch.load(
                os.path.join(CLIPfeature_save_path, filename.split("/")[-1])
            ).numpy()
        ...
        if max_position_length is None:
            feature = np.concatenate(
                (feature, CLIP_feature[None, ...].repeat(feature.shape[0], axis=0)),
                axis=-1,
            )
            video_features[video_id] = feature
        else:
            ...
            new_feature = np.concatenate(
                (
                    new_feature,
                    CLIP_feature[None, ...].repeat(new_feature.shape[0], axis=0),
                ),
                axis=-1,
            )
            video_features[video_id] = new_feature
...
def load_video_features_concatCLIPfeature_v2(
    root, max_position_length, CLIPfeature_save_path, use_normforvideofeature
):
    video_features = dict()
    extension = "*.pt"
    count = 0
    ...
    for filename in tqdm(filenames, total=len(filenames), desc="load video features"):
        ...
        if use_normforvideofeature:
            # print('use_normforvideofeature')
            feature = l2_normalize_np_array(feature)
            CLIP_feature = l2_normalize_np_array(CLIP_feature)
        if max_position_length is None:
            feature = np.concatenate((feature, CLIP_feature), axis=-1)
            video_features[video_id] = feature
        else:
            new_feature = visual_feature_sampling(
                feature, max_num_clips=max_position_length
            )
            CLIP_feature = visual_feature_sampling(
                CLIP_feature, max_num_clips=max_position_length
            )
            new_feature = np.concatenate((new_feature, CLIP_feature), axis=-1)
            video_features[video_id] = new_feature
            # if new_feature.shape[0] != feature.shape[0]:
            #    print(f"Reduced: {feature.shape[0]} --> {new_feature.shape[0]}")
    return video_features
