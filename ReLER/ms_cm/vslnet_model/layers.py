...
class WeightedPool(nn.Module):
    ...
    def forward(self, x, mask):
        ...
        # x：(batch_size, seq_length, dim) weight：（dim,1）
        # 输出就是(seq_length,dim)与（dim，1）做矩阵乘法
...
class NPM(nn.Module):
    ...
    def forward(self, cafeature, camask):
        # print('cafeature.shape: {}'.format(cafeature.shape))
        pooled_ca = self.weighted_pool(cafeature, camask)  # (batch_size, dim)
        # print('pooled_ca.shape: {}'.format(pooled_ca.shape))
        logits = self.linear(pooled_ca)
        # logits = mask_logits(logits, camask)

        NPM_scores = nn.Sigmoid()(logits)
        # print('NPM_scores.shape: {}'.format(NPM_scores.shape))
        return NPM_scores
...
class ConditionedPredictor(nn.Module):
    def __init__(self, dim, num_heads, max_pos_len, drop_rate=0.0, predictor="rnn"):
        ...
        elif predictor == "justfc":
            pass

        self.start_block = nn.Sequential(
            Conv1D(
                in_dim=(2 * dim)
                if self.predictor == "rnn" or self.predictor == "bert"
                else dim,
                out_dim=dim,
                kernel_size=1,
                stride=1,
    ...
    def forward(self, x, mask):
        ...
        elif self.predictor == "justfc":
            start_features = self.start_block(x)  # (batch_size, seq_len, 1)
            end_features = self.end_block(x)

        # print('end_features: {}'.format(end_features.shape))
        start_logits = mask_logits(start_features.squeeze(2), mask=mask)
        end_logits = mask_logits(end_features.squeeze(2), mask=mask)
        # print('end_logits: {}'.format(end_logits.shape))
        # exit()
