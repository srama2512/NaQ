...
def pad_sequences_1d_video_numscale(
    sequences,
    dtype=torch.long,
    ...
    split_len = max_length // cur_scale
    start_point_list = []
    ...
    if isinstance(sequences[0], torch.Tensor):
        ...
        scale_mask = torch.ones(
            (len(sequences), cur_scale), dtype=torch.float32, device=device
        )
    else:  # np
        assert "numpy" in str(dtype), "dtype and input type does not match"
        padded_seqs = np.zeros((len(sequences), max_length) + extra_dims, dtype=dtype)
    ...
    for idx, seq in enumerate(sequences):
        ...
        padded_seqs[idx, :end] = seq
        mask[idx, :end] = 1
        for i in range(cur_scale - 1, -1, -1):
            if end < start_point_list[i]:
                scale_mask[idx, i] = 0

    return padded_seqs, mask, scale_mask  # , lengths
