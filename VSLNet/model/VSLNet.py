...
class VSLNet(nn.Module):
    ...
    def forward(
        ...
        if get_losses:
            ...
            highlight_loss = self.compute_highlight_loss(
                h_score, labels["h_labels"], v_mask
            )
            loc_loss = self.compute_loss(
                start_logits, end_logits, labels["s_labels"], labels["e_labels"]
            )
            return h_score, start_logits, end_logits, highlight_loss, loc_loss
        else:
