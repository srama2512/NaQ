...
class FeatureEncoder(nn.Module):
    def __init__(
        ...
        self.num_extra_attn_blocks = num_extra_attn_blocks
        self.extra_attention_blocks = None
        if num_extra_attn_blocks > 0:
            self.extra_attention_blocks = nn.ModuleList()
            for _ in range(num_extra_attn_blocks):
                self.extra_attention_blocks.append(
                    MultiHeadAttentionBlock(
                        dim=dim, num_heads=num_heads, drop_rate=drop_rate
                    )
                )
            print("Adding {} extra attention blocks".format(num_extra_attn_blocks))
    ...
    def forward(self, x, mask=None):
        ...
        if self.num_extra_attn_blocks > 0:
            for i in range(self.num_extra_attn_blocks):
                features = self.extra_attention_blocks[i](features, mask=mask)
        return features
